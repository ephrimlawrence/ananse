...
// Client main menu
MenuRouter.menu(MenuType.customer)
...
MenuRouter.menu(MenuType.customer_registration) // menu name becomes form name
	.isForm()
	.inputs([
		{
			name: "first_name",
		...
		},
		{
			...
			handler: async (_req) => {
				...
				const form = await _req.session.get<any>(
					MenuType.customer_registration,
				);
...
MenuRouter.menu("client_created").message(
