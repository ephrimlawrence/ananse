...
export abstract class BaseSessionCache {
	...
	async configure(options?: CacheOptions): Promise<void> {
	...
	abstract get<T>(sessionId: string, key: string, defaultValue?: T): Promise<T>;

	abstract getAll<T = unknown>(sessionId: string): Promise<T>;
}
